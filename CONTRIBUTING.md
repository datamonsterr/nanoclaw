...
NanoClaw uses [Claude Code skills](https://code.claude.com/docs/en/skills) — markdown files with optional supporting files that teach Claude how to do something. These skills are also compatible with [OpenCode](https://opencode.ai), which reads `.claude/skills/` natively. There are four types of skills in NanoClaw, each serving a different purpose.
